...
#'
#' @description
#' Computes the implied volatility of an option by inverting the Black-Scholes formula using Newton-Raphson.
#'
#' @param market_price  numeric scalar: observed market price of the option
#' @param S             numeric scalar: current price of the underlying asset
#' @param K             numeric scalar: strike price of the option
#' @param T             numeric scalar: time to expiration (in years)
#' @param r             numeric scalar: risk-free interest rate (annualized)
#' @param is_call       integer scalar: 1 for call, 0 for put
#'
#' @return
...
implied_vol <- function(market_price, S, K, T, r, is_call) {
    .Call("r_implied_vol",
        as.double(market_price), as.double(S), as.double(K), as.double(T), as.double(r), as.integer(is_call),
        PACKAGE = "Volaris")
}